...
def calcAccuracycDS1(i):
    return round(classification(woNaN(ds), dsmodels['Model'].iloc[i], dsmodels['Parameter Grid'].iloc[i]),
                 4) * 100
...
def calcAccuracycDS3(i):
    return round(classification(woOutliersMean(ds), dsmodels['Model'].iloc[i], dsmodels['Parameter Grid'].iloc[i]),
                 4) * 100


# calls classification methods for fourth ds and rounds
def calcAccuracycDS4(i):
    return round(classification(woOutliersIQR(ds), dsmodels['Model'].iloc[i], dsmodels['Parameter Grid'].iloc[i]),
                 4) * 100

...
def calcAccuracyDS2(i):
    return round(r.regression(woNaNOutliers(ds),dsmodels['Model'].iloc[i],dsmodels['Parameter Grid'].iloc[i]),4)*100

def calcAccuracyDS3(i):
    return round(r.regression(woOutliersMean(ds),dsmodels['Model'].iloc[i],dsmodels['Parameter Grid'].iloc[i]),4)*100

def calcAccuracyDS4(i):
    return round(r.regression(woOutliersIQR(ds),dsmodels['Model'].iloc[i],dsmodels['Parameter Grid'].iloc[i]),4)*100